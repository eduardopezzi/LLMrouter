...
class TestArgParsing:
    ...
    def test_parse_args_benchmarks_refresh(self) -> None:
        with patch.object(sys, "argv", ["llmrouter", "benchmarks-refresh", "--check"]):
            args = _parse_args()
...
class TestMainCommands:
    ...
    def test_main_export_contracts(
    ...
    def test_main_diff_contracts(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        f1 = tmp_path / "prev.json"
        f2 = tmp_path / "curr.json"
        f1.write_text(json.dumps({"schema_version": "1.0"}))
        f2.write_text(json.dumps({"schema_version": "2.0"}))
    ...
    def test_main_panel_stats(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        models_file = tmp_path / "models.yaml"
        models_file.write_text("models:\n  - name: m1\n    provider: openai\n    tier: 1\n")
    ...
    def test_main_health_json(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch.object(
            sys,
            "argv",
            [
    ...
    def test_main_semantic_inspect_json(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch.object(
            sys,
            "argv",
            [
    ...
    def test_main_health_text(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch.object(
            sys,
            "argv",
            [
